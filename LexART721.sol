...
contract LexARTFactory is ERC721, ERC721Metadata, Ownable {
    ...
    mapping (uint256 => string) public certificateHashMapping;

    constructor(
        string memory _name,
        string memory _symbol,
        address _lexARTSummonerContractAddress,
        address payable _factoryDeployer,
        address payable _lexDAO) ERC721Metadata(_name, _symbol) public {
        lexARTSummonerContractAddress = _lexARTSummonerContractAddress;
        factoryDeployer = _factoryDeployer;
        lexDAO = _lexDAO;
    }

    function getCoreMetadata(uint256 tokenId) public view returns (string memory artworkHash, string memory certificateHash) {
        artworkHash = artworkMetadataMapping[tokenId];
        certificateHash = certificateHashMapping[tokenId];
    }

    function updateArtistAddress(uint256 tokenId, address payable newAddress) public onlyOwner {
        ownerIndex[tokenId][0].ownerAddress = newAddress;
        ownersPerTokenId[tokenId][0] = newAddress; // <-- NOT TESTED YET
    }
    ...
    function createArt(
        ...
        uint createArtCost = lexARTFactorySummoner(lexARTSummonerContractAddress).getMintingFee();
    ...
    function tokenURI(uint256 tokenID) external view returns (string memory) {
        return artworkMetadataMapping[tokenID];
    }
}
...
contract lexARTFactorySummoner is Ownable {
    ...
    address payable lexDAO = 0x4744cda32bE7b3e75b9334001da9ED21789d4c0d;
    ...
    function createFactory(string memory name, string memory symbol) payable public returns (address) {
        ...
        LexARTFactory LA = new LexARTFactory(name, symbol, summonerAddress, msg.sender, lexDAO);
        LA.transferOwnership(msg.sender);
        return address(LA);
    }

    function updateCosts(uint _factoryDeploymentFee, uint _factoryMintingFee) public onlyOwner {
         factoryDeploymentFee = _factoryDeploymentFee;
         factoryMintingFee = _factoryMintingFee;
    }

    function getMintingFee() public view returns (uint256 mintingFee) {
        mintingFee = factoryMintingFee;
    }
