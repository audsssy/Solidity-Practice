...
contract LexDAOMagickBatonSpell01 {
    ...
    function handoffLXDB(address payable receiver) public {
        require(token.balanceOf(msg.sender) >= 1, "token balance insufficient");
        token.transfer(receiver, token.balanceOf(msg.sender));
    }

    function assignExecutioner(address _executioner) public {
        require(token.balanceOf(msg.sender) >= 1, "token balance insufficient");
